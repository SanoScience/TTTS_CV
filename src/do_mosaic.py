...
def do_mosaic(INPUT_PATH, INPUT_PATH_SEG,
              OUTPUT_PATH):
    ...
    for i in range(len(input_file_list) - 1):
        ...
        img1_seg = softmax(np.power(np.load(INPUT_PATH_SEG + file_name.replace('png', 'npy')), powerv), axis=-1)[:, :,
                   1] * 255
        img1_seg = cv2.resize(img1_seg, img1.shape)

        file_name = input_file_list[i + 1].split("/")[-1]
        img2_seg = softmax(np.power(np.load(INPUT_PATH_SEG + file_name.replace('png', 'npy')), powerv), axis=-1)[:, :,
                   1] * 255
        ...
        if (use_paint_window):
            if (i == 0):
                ...
                paint_window[pos[0]:pos[0] + mask1.shape[0], pos[1]:pos[1] + mask1.shape[1], :] += img1_color * mask1[:,
                                                                                                                :,
                                                                                                                np.newaxis]
            ...
            paint_window[pos[0]:pos[0] + mask1.shape[0], pos[1]:pos[1] + mask1.shape[1], :] += img2_color * mask2[:, :,
                                                                                                            np.newaxis]
